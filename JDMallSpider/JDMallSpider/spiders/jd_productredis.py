...
"""
scrapy-redis: 分布式爬虫
1.修改爬虫类的继承关系: scrapy_redis.spiders.RedisSpider
2.指定redis-key的值: 用于指定起始url列表,在Redis数据库中的key
3.把重写start_requests方法 改成 重写make_request_from_data方法
"""


# 1.修改爬虫继承类scrapy_redis.spiders.RedisSpider
class JdProductRedisSpider(RedisSpider):
    name = 'jd_productredis'
    ...
    # 2.指定redis-key的值,如 '项目名称:爬虫名称'
    redis_key = 'JDMallSpider:jd_productredis'

    # 3.把重写的start_requests方法改成重写make_request_from_data方法
    def make_request_from_data(self, data):
        """
        根据Redis数据库读取分类信息的二进制数据,构建请求
